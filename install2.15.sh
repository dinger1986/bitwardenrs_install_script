...
wget https://github.com/dani-garcia/bitwarden_rs/archive/1.15.1.tar.gz
tar -xzvf 1.15.1.tar.gz
mv bitwarden_rs-1.15.1 bitwarden_rs
cd bitwarden_rs
cargo build --features sqlite --release
cd ..

#Clone and checkout repository
wget https://github.com/dani-garcia/bw_web_builds/releases/download/v2.15.1/bw_web_v2.15.1.tar.gz
tar -xzvf bw_web_v2.15.1.tar.gz

#Create BitwardenRS folder, copy and tidy
sudo mkdir /opt/bitwardenrs
sudo cp -r ~/bitwarden_rs/target/release/bitwarden_rs /opt/bitwardenrs
sudo mv ~/web-vault /opt/bitwardenrs/web-vault
...
echo "Press any key to reboot, we need to do this to kill the temporary bitwardenrs process"
while [ true ] ; do
read -t 3 -n 1
if [ $? = 0 ] ; then
exit ;
